...
router.post("/create-super-admin", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO users (name, email, password, role) VALUES ($1, $2, $3, 'super_admin') RETURNING *`,
      [name, email, hashedPassword],
    );

    const { password: pwd, ...superAdminWithoutPassword } = result.rows[0];
    res.status(201).json({ success: true, message: "Super admin created successfully", data: superAdminWithoutPassword });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
});
...
router.post("/verify-token", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT id, ref_no, name, email, role, created_at FROM users WHERE id = $1`,
      [decoded.id],
    );
    ...
    res.status(200).json({ success: true, message: "Token is valid", role: result.rows[0].role, data: result.rows[0] });
  } catch (error) {
...
router.post("/refresh-token", async (req, res) => {
  try {
    ...
    return res.status(200).json({ success: true, message: "Token refreshed successfully", token: newToken });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
});
...
router.post("/logout", async (req, res) => {
  try {
    ...
    if (accessToken) {
      ...
      try {
        await pool.query(
          `INSERT INTO blacklisted_tokens (token, user_id, expires_at) VALUES ($1, $2, $3) ON CONFLICT (token) DO NOTHING`,
          [accessToken, userId, expiresAt]
        );
      } catch (_) {}

      if (refreshToken && userId) {
        await pool.query("DELETE FROM refresh_tokens WHERE token = $1 AND user_id = $2", [refreshToken, userId]);
...
router.post("/change-password", async (req, res) => {
  try {
    ...
    await pool.query("UPDATE users SET password = $1, updated_at = NOW() WHERE id = $2", [hashedPassword, decoded.id]);
