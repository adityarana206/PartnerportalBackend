...
const Complaint = {
  ...
  async create(data) {
    const result = await pool.query(
      `INSERT INTO complaints (
        ...
        sender_type, partner_type, sender_id, sender_name, message_text,
        change_details, message_timestamp, direction, status
  ...
  async findAll() {
    const result = await pool.query("SELECT * FROM complaints ORDER BY message_timestamp DESC");
    return result.rows;
  },
  ...
  async findByPartnerNo(partnerNo) {
    const result = await pool.query(
      "SELECT * FROM complaints WHERE sender_id = $1 ORDER BY message_timestamp DESC",
      [partnerNo]
    );
    return result.rows;
  },
  ...
  async findByThreadId(threadId) {
    const result = await pool.query(
      "SELECT * FROM complaints WHERE thread_id = $1 ORDER BY message_timestamp ASC",
      [threadId]
    );
    return result.rows;
  },
